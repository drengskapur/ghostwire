...
main() {
    ...
    if [ "${USE_LOCAL_CHART:-false}" = "true" ]; then
        log "Installing Ghostwire via Helm (local chart)..."
        ...
        log "Installing Ghostwire via Helm (OCI registry with ${VERSION})..."
...
EOF
npm install --no-save playwright@1.56.1 && node test.js
'
